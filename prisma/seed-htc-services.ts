...
async function main() {
  console.log(`Start seeding HTC wellness services ...`);

  // Healthcare Therapy Center services based on https://htcwellness.com/services
  const htcServicesData = [
    {
  ...
  // Create services
  for (const service of htcServicesData) {
    ...
    console.log(
      `Created HTC service with id: ${createdService.id} - ${service.title}`
    );
  }

  console.log(`HTC wellness services seeding finished.`);
}
