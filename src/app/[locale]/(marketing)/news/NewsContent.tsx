"use client";
import React, { useEffect, useState, useMemo, useTransition } from "react";
import Image from "next/image";
import { Link, useRouter, usePathname } from "@/navigation";
import { useSearchParams } from "next/navigation";
import { DisplayNews, News, NewsResponse } from "@/types/post";
import { getLocalizedNews } from "@/utils";
import { htmlToTextAndTruncate } from "@/lib/utils";

const DEFAULT_NEWS_IMAGE = "/images/default_news_ai.jpeg";

function LoadingSkeleton() {
  return (
    <div className="animate-pulse">
      {/* Trending Topics Skeleton */}
      <section className="container mx-auto px-4 mb-16 md:mb-20 max-w-[1040px]">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="md:w-1/2">
            <div className="relative rounded-lg overflow-hidden block h-full bg-gray-200" style={{height:304}}></div>
          </div>
          <div className="md:w-1/2">
            <div className="grid grid-cols-2 gap-4 h-full">
              {[1,2,3,4].map(i => (
                <div key={i} className="relative rounded-lg overflow-hidden bg-gray-200 aspect-square"></div>
              ))}
            </div>
          </div>
        </div>
      </section>
      {/* Post Listing Skeleton */}
      <section className="container mx-auto px-4 max-w-[1040px]">
        {[1,2,3,4,5].map(i => (
          <article key={i}>
            <div className="relative flex flex-col md:flex-row items-center mb-4 md:mb-12 group justify-between">
              <div className="pr-8 flex flex-col w-full">
                <div className="h-8 bg-gray-200 mb-4 w-3/4"></div>
                <div className="h-20 bg-gray-200 mb-4"></div>
              </div>
              <div className="mt-4 md:mt-0">
                <div className="block relative rounded-xl overflow-hidden aspect-square md:h-44 bg-gray-200"></div>
              </div>
            </div>
            <div className="flex justify-center w-full">
              <div className="w-full mx-10 md:mx-30 border-b border-[#0000004D] mb-8"></div>
            </div>
          </article>
        ))}
      </section>
      {/* Pagination Skeleton */}
      <section className="container mx-auto px-4 mb-12 flex justify-start max-w-[1040px]">
        <div className="flex items-center gap-2">
          {[1,2,3,4].map(i => (
            <div key={i} className="w-10 h-10 bg-gray-200 rounded"></div>
          ))}
        </div>
      </section>
      {/* Newsletter Skeleton */}
      <section className="py-16 bg-white max-w-[1040px] mx-auto">
        <div className="container mx-auto px-4 text-center">
          <div className="h-12 md:h-16 w-96 bg-gray-200 rounded mx-auto mb-6"></div>
          <div className="max-w-3xl mx-auto mb-8 space-y-3">
            <div className="h-6 bg-gray-200 rounded"></div>
            <div className="h-6 bg-gray-200 rounded w-4/5 mx-auto"></div>
          </div>
          <div className="h-12 w-48 bg-gray-200 rounded-full mx-auto"></div>
        </div>
      </section>
    </div>
  );
}

export default function NewsContentClient({ locale, t, appointmentLink }: { locale: string; t: any; appointmentLink: string }) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();
  const [isPending, startTransition] = useTransition();
  const [loading, setLoading] = useState(true);
  const [trendingNews, setTrendingNews] = useState<DisplayNews[]>([]);
  const [newsItems, setNewsItems] = useState<DisplayNews[]>([]);
  const [totalNews, setTotalNews] = useState(0);
  const postsPerPage = 10;
  const page = searchParams.get("page") || "1";
  const currentPage = Number(page);

  useEffect(() => {
    let ignore = false;
    setLoading(true);
    Promise.all([
      fetch(`/api/news/featured`).then(res => res.ok ? res.json() : { posts: [] }),
      fetch(`/api/news?page=${currentPage}&limit=${postsPerPage}&status=PUBLISHED&pin=false`).then(res => res.ok ? res.json() : { news: [], meta: { total: 0 } })
    ]).then(([featuredData, data]) => {
      if (ignore) return;
      setTrendingNews((featuredData.posts || []).map((newsItem: any) => getLocalizedNews(newsItem, locale)));
      setTotalNews(data.meta?.total || 0);
      setNewsItems((data.news || []).map((newsItem: News) => getLocalizedNews(newsItem, locale)));
      setLoading(false);
    });
    return () => { ignore = true; };
  }, [currentPage, locale]);

  // Pagination logic
  const totalPages = useMemo(() => Math.ceil(totalNews / postsPerPage), [totalNews]);
  const getPageNumbers = () => {
    const pageNumbers = [];
    pageNumbers.push(1);
    const startPage = Math.max(2, currentPage - 1);
    const endPage = Math.min(totalPages - 1, currentPage + 1);
    if (startPage > 2) pageNumbers.push("...");
    for (let i = startPage; i <= endPage; i++) pageNumbers.push(i);
    if (endPage < totalPages - 1) pageNumbers.push("...");
    if (totalPages > 1) pageNumbers.push(totalPages);
    return pageNumbers;
  };
  const pageNumbers = getPageNumbers();

  // Handle page navigation
  const goToPage = (pageNum: number) => {
    startTransition(() => {
      router.push(`${pathname}?page=${pageNum}`);
    });
  };

  if (loading || isPending) return <LoadingSkeleton />;

  if (newsItems.length === 0) {
    return (
      <section className="container mx-auto px-4 mb-16 md:mb-20 max-w-[1040px] text-center py-20">
        <h2 className="text-2xl text-gray-600 mb-4">{t.emptyState.title}</h2>
        <p className="text-gray-500">{t.emptyState.description}</p>
      </section>
    );
  }

  return (
    <>
      {/* Trending Topics */}
      {trendingNews.length > 0 && (
        <section className="container mx-auto px-4 mb-16 md:mb-20 max-w-[1040px]">
          <div className="flex flex-col md:flex-row gap-4 md:h-[480px]">
            <div className="h-[304px] md:h-full md:w-1/2">
              <Link
                href={`/news/${trendingNews[0].slug}`}
                className="group relative rounded-lg overflow-hidden block h-full"
              >
                <div className="aspect-auto h-full relative">
                  <Image
                    src={trendingNews[0].image || DEFAULT_NEWS_IMAGE}
                    alt={trendingNews[0].title}
                    fill
                    className="object-cover group-hover:scale-105 transition-transform duration-300"
                  />
                  <div className="absolute inset-0">
                    <div className="absolute bottom-0 w-full p-4 md:p-8 bg-[#00000080]">
                      <h3 className="text-white font-medium text-xl md:text-2xl">
                        {trendingNews[0].title}
                      </h3>
                    </div>
                  </div>
                </div>
              </Link>
            </div>
            <div className="md:w-1/2">
              <div className="grid grid-cols-2 gap-4 h-full">
                {trendingNews.slice(1, 5).map((newsItem) => (
                  <Link
                    href={`/news/${newsItem.slug}`}
                    key={newsItem.id}
                    className="group relative rounded-lg overflow-hidden"
                  >
                    <div className="aspect-square relative">
                      <Image
                        src={newsItem.image || DEFAULT_NEWS_IMAGE}
                        alt={newsItem.title}
                        fill
                        className="object-cover group-hover:scale-105 transition-transform duration-300"
                      />
                      <div className="absolute inset-0">
                        <div className="absolute bottom-0 p-4 bg-[#00000080]">
                          <h3 className="text-white font-medium text-sm md:text-base">
                            {newsItem.title}
                          </h3>
                        </div>
                      </div>
                    </div>
                  </Link>
                ))}
              </div>
            </div>
          </div>
        </section>
      )}
      {/* Topic Listing */}
      <section className="container mx-auto px-4 max-w-[1040px]">
        {newsItems.map((newsItem) => (
          <article key={newsItem.id}>
            <div className="relative flex flex-col md:flex-row items-start md:items-center mb-4 md:mb-12 group justify-between">
              <Link
                href={`/news/${newsItem.slug}`}
                className="absolute inset-0 z-1"
              />
              <div className="pr-8 flex flex-col">
                <h2 className="text-2xl font-medium text-[#222222] mb-4 group-hover:text-[#B1873F] transition-colors">
                  {newsItem.title}
                </h2>
                <p className="text-gray-600 line-clamp-3 mb-4">
                  {htmlToTextAndTruncate(newsItem.description, 200)}
                </p>
              </div>
              <div className="mt-4 md:mt-0">
                <Link
                  href={`/news/${newsItem.slug}`}
                  className="block relative rounded-xl overflow-hidden aspect-square md:h-44"
                >
                  <Image
                    src={newsItem.image || DEFAULT_NEWS_IMAGE}
                    alt={newsItem.title}
                    fill
                    className="object-cover"
                  />
                </Link>
              </div>
            </div>
            <div className="flex justify-center w-full">
              <div className="w-full mx-10 md:mx-30 border-b border-[#0000004D] mb-8"></div>
            </div>
          </article>
        ))}
      </section>
      {/* Pagination */}
      <section className="container mx-auto px-4 mb-12 flex justify-start max-w-[1040px]">
        <div className="flex items-center gap-2">
          {/* Previous page button */}
          <button
            onClick={() => currentPage > 1 && goToPage(currentPage - 1)}
            className={`flex items-center justify-center ${currentPage === 1 ? "text-gray-300 cursor-not-allowed" : "hover:text-[#B1873F] cursor-pointer"}`}
            aria-label={t.pagination.previousPage}
            disabled={currentPage === 1}
            type="button"
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M15 18L9 12L15 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          {/* Page numbers */}
          {pageNumbers.map((pageNumber, index) => (
            <React.Fragment key={index}>
              {pageNumber === "..." ? (
                <span className="w-10 h-10 flex items-center justify-center text-gray-600">...</span>
              ) : (
                <button
                  onClick={() => goToPage(Number(pageNumber))}
                  className={`w-10 h-10 rounded flex items-center justify-center border ${currentPage === pageNumber ? "border-[#B1873F] text-[#B1873F]" : "border-gray-300 bg-white text-gray-600"} font-bold cursor-pointer`}
                  aria-label={`${t.pagination.page} ${pageNumber}`}
                  aria-current={currentPage === pageNumber ? "page" : undefined}
                  type="button"
                >
                  {pageNumber}
                </button>
              )}
            </React.Fragment>
          ))}
          {/* Next page button */}
          <button
            onClick={() => currentPage < totalPages && goToPage(currentPage + 1)}
            className={`flex items-center justify-center ${currentPage >= totalPages ? "text-gray-300 cursor-not-allowed" : "hover:text-[#B1873F] cursor-pointer"}`}
            aria-label={t.pagination.nextPage}
            disabled={currentPage >= totalPages}
            type="button"
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M9 18L15 12L9 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
        </div>
      </section>
      {/* Newsletter */}
      <section className="bg-white py-16 md:py-20">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-5xl max-text-[51px] font-semibold text-[#1F1F1F] mb-6">
            {t.cta.heading}
          </h2>
          <p className="text-lg md:text-xl text-black mb-10 max-w-3xl mx-auto md:px-16">
            {t.cta.subheading}
          </p>
          <a
            href={appointmentLink}
            target="_blank"
            className="inline-block relative rounded-3xl px-6 py-3 bg-[#B1873F] text-white font-medium hover:bg-[#9e7736] transition-colors"
          >
            <span className="absolute inset-0 bg-[#a75e24] z-0 rounded-full animate-heath-beat"></span>
            <span className="relative z-10">{t.cta.button}</span>
          </a>
        </div>
      </section>
    </>
  );
}
